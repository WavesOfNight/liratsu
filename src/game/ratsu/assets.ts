...
export function loadRatsuAssets(): Promise<RatsuAssets> {
  if (!cache) {
    cache = (async () => {
      ...
      const [up, down, left, right, blob, downLeft, downRight, upLeft, upRight, red, stone, wood, bad, shop, itemroom, bombIcon, bomb1, bomb2, bombContainer, coin] = await Promise.all([
        playerFrames('Up'),
        playerFrames('Down'),
        playerFrames('Left'),
        playerFrames('Right'),
